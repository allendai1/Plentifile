...
export default function DownloadFileRow(props){
    ...
    return(
        ...
        <div className="">{props.e.metadata.name}</div>
            <div className="file-row-size-label">{ bytesToSize(props.e.metadata.size)}</div>
            


            <svg title="download" onClick={downloadFile} xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><path fill="#1F1F23" d="M16 11h5l-9 10-9-10h5v-11h8v11zm1 11h-10v2h10v-2z"><title>Download</title></path></svg>
        </div>
    )
}