...
export default function MyFiles() {
  ...
  const databaseUserLinksRef = database.users.doc(currentUser.uid).collection("links")
  ...
  async function loadFile(){
    console.log("Load File")
      await databaseUserLinksRef.get().then(coll=>{
        const sorted = coll.docs.sort((a, b) => b.data().preciseDate.toDate()-a.data().preciseDate.toDate());
        setDataFetched(true)
        setArr([...sorted])
  ...
  function deleteFile(e) {
    ...
    selected.forEach(async fileid=>{
      ...
      database.users.doc(currentUser.uid).update({
        ...
        totalFileSize : firebase.firestore.FieldValue.increment(-(arr.filter(x=>x.id===fileid))[0].data().size)
    })
      storage.ref(`${fileid}`).list().then((x)=>{
  ...
  function sortByDate(e){
    ...
    if(dateSort==="ascending"){
      const sorted = arr.sort((a, b) => a.data().preciseDate.toDate()-b.data().preciseDate.toDate());
      setArr([...sorted])
      setDateSort("descending")
      setArrow([0,1,0])
    }
    else{
      const sorted = arr.sort((a, b) => b.data().preciseDate.toDate()-a.data().preciseDate.toDate());
      setArr([...sorted])
      ...
      <div className='files-container'>
        ...
        <div className="files-inner-header">
          ...
          <div className="header-name" onClick={sortByName}>
            Name
           <svg className="svg-up" orientation={arrowOrientation[0]} viewBox="0 0 20 20">
							<path fill="none" d="M11.611,10.049l-4.76-4.873c-0.303-0.31-0.297-0.804,0.012-1.105c0.309-0.304,0.803-0.293,1.105,0.012l5.306,5.433c0.304,0.31,0.296,0.805-0.012,1.105L7.83,15.928c-0.152,0.148-0.35,0.223-0.547,0.223c-0.203,0-0.406-0.08-0.559-0.236c-0.303-0.309-0.295-0.803,0.012-1.104L11.611,10.049z"></path>
						</svg>
           </div>
          <div className='header-desc'>Description</div>
          <div className='header-date' onClick={sortByDate}>Date Created
          <svg className="svg-up" orientation={arrowOrientation[1]} viewBox="0 0 20 20">
							<path fill="none" d="M11.611,10.049l-4.76-4.873c-0.303-0.31-0.297-0.804,0.012-1.105c0.309-0.304,0.803-0.293,1.105,0.012l5.306,5.433c0.304,0.31,0.296,0.805-0.012,1.105L7.83,15.928c-0.152,0.148-0.35,0.223-0.547,0.223c-0.203,0-0.406-0.08-0.559-0.236c-0.303-0.309-0.295-0.803,0.012-1.104L11.611,10.049z"></path>
						</svg>
          </div>
          <div className='header-size'  onClick={sortBySize}>Size
          <svg className="svg-up" orientation={arrowOrientation[2]} viewBox="0 0 20 20">
							<path fill="none" d="M11.611,10.049l-4.76-4.873c-0.303-0.31-0.297-0.804,0.012-1.105c0.309-0.304,0.803-0.293,1.105,0.012l5.306,5.433c0.304,0.31,0.296,0.805-0.012,1.105L7.83,15.928c-0.152,0.148-0.35,0.223-0.547,0.223c-0.203,0-0.406-0.08-0.559-0.236c-0.303-0.309-0.295-0.803,0.012-1.104L11.611,10.049z"></path>
						</svg>
