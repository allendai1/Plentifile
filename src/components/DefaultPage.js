		...
		await storage
			...
			.then((x) => {
				...
				x.items.forEach(async (file, index) => {
					...
					setFileList((oldList) =>
						[...oldList, fileObj].sort((a, b) => (a.name > b.name ? 1 : -1))
					);
				});
			});
		setLoading(false);
		...
		<>
				...
				<div className="password_form">
					<Card>
						...
						<Card.Body>
							...
							<br />
							<Button className="w-100" onClick={submit} variant="primary">
								Enter
							</Button>
						</Card.Body>
					</Card>
			...
			{pwprotected === false && !loading && (
				<div className="files-page">
					...
					<div className="files-gap">
						{fileList.map((e, index) => {
							return <DownloadFileRow key={index} e={e} />;
						})}
					</div>
				</div>
			)}
